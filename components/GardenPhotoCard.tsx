...
export default function GardenPhotoCard({
  ...
  return (
    <div className="bg-white rounded-3xl shadow-[0_10px_40px_rgb(0,0,0,0.08)] overflow-hidden group hover:shadow-[0_20px_50px_rgb(236,72,153,0.15)] transition duration-500 relative border border-pink-100/50">
      <div
        className={`relative aspect-[3/4] overflow-hidden bg-pink-50/50 transition-colors duration-500 ${isLoaded ? "bg-transparent" : ""}`}
      >
        <Image
          src={photo.url}
          alt={photo.caption}
          ...
          className={`object-cover transition-all duration-700 ease-out group-hover:scale-[1.03] ${isLoaded ? "opacity-100 scale-100" : "opacity-0 scale-[1.02] blur-sm"}`}
        />
